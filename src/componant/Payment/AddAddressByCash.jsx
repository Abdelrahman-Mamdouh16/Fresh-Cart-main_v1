...
export default function AddAddressByCash() {
    ...
    let { CheckOut, CartID, setCartVal ,setNumOfCartItems} = useContext(CartContext)

    async function SubmitForm(val) {
        setCartVal(val);
        // let res = await CheckOut(CartID,val);
        await CheckOut(CartID, val)
        if(val)
        {setNumOfCartItems(0)}
        navigate('PayCash')
    }
    ...
    return (
        <>
            <form className="AddAddressByCash my-5" onSubmit={Formik.handleSubmit}>
                <div className="container">
                    ...
                    <div className='d-flex justify-content-center my-4'>
                        ...
                        <button className='btn bg-main text-white' onClick={PayByOnline}>Pay Online</button>
                    </div>
                    ...
                    <input type="submit" className='form-control mt-4 w-75 d-block m-auto btn btn-outline-success' value="Pay Cash" />
                </div>
            </form>
        </>
    )
